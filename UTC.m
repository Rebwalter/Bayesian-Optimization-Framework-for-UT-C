function [Results2m,UTCI]=UTC(Sensitivity,m,MeteoDataBS,Name_Site)
...
n			=	size(MeteoDataBS,1);        % Calculation length, there is no need to change this
Name_SiteFD	=	Name_Site;		            % Name for UEHMForcingData
Name_ViewFactor = Name_Site;
OPTION_RAY	=	0;                          % Load precalculated view factors [1], Recalculate view factors [0]


MeteoDataBS.Windspeed(MeteoDataBS.Windspeed(1:n,:)==0) = 0.01;	% Wind speed cannot be 0 otherwise the resistance function fails


MeteoDataRaw	=	struct('LWR_in',MeteoDataBS.LWRin(1:n,:),'SAB1_in',MeteoDataBS.SAB1(1:n,:),...
					'SAB2_in',MeteoDataBS.SAB2(1:n,:),'SAD1_in',MeteoDataBS.SAD1(1:n,:),...
					'SAD2_in',MeteoDataBS.SAD2(1:n,:),'T_atm',MeteoDataBS.Tatm(1:n,:),...
					'windspeed_u',MeteoDataBS.Windspeed(1:n,:),'pressure_atm',MeteoDataBS.Pressure_Pa(1:n,:),...
					'rain',MeteoDataBS.Precipitation(1:n,:),'rel_humidity',MeteoDataBS.RelativeHumidity(1:n,:)./100,...
					'Date',MeteoDataBS.Time(1:n,:));


clearvars -except Sensitivity MeteoDataRaw n m Name_Site OPTION_RAY NameOutput Name_SiteFD LAI_TimeSeries Name_ViewFactor OutputsToSave
...
[TempVec,TempVecNames,TempDamp,TempDampNames,Humidity,HumidityNames,...
    ...
    ...
    MeteoData,ParSoil]=...
    helper.InitializeOutputVariablesSens(MeteoDataRaw,n,m,Name_Site,Name_SiteFD,LAI_TimeSeries);
...
% Start runs with varying inputs
for ittm = 1:m

[WallLayers,ParSoilRoof,ParSoilGround,ParInterceptionTree,...
	PropOpticalTree,...
	ParThermalBulidingInt,ParThermalTree,...
	ParVegRoof,ParVegGround,ParVegTree,...
    PropOpticalRoof,PropOpticalIndoors,ParWindows,ParHVAC,BEM_on]...
    =feval(strcat('data_functions.Data_UEHM_site_',Name_Site),MeteoData,1,LAI_TimeSeries);

% Get LHS row for ittm run
[Geometry_m,ParTree,geometry,...
    FractionsRoof,FractionsGround,PropOpticalWall,...
    PropOpticalGround,Person,ParThermalRoof,...
    ParThermalGround,ParThermalWall]...
    = helper.SensitivityInput_HST(MeteoData,Sensitivity,ittm,ParVegGround,ParVegRoof);



% Check input parameters
helper.InputCheck_all(Geometry_m,ParTree,Person,MeteoData.Zatm,FractionsRoof,FractionsGround,ParVegRoof,ParVegGround,ParVegTree)

% Calculate view factors
[ViewFactor,ViewFactorPoint]=ray_tracing.VFUrbanCanyon(OPTION_RAY,Name_ViewFactor,Geometry_m,geometry,Person,ParTree);
...
[TempVec_ittm,Int_ittm,ExWater_ittm,Vwater_ittm,Owater_ittm,SoilPotW_ittm,...
    ...
    ]=helper.CreateVariablesOfPreviousTimeStep(ittm,ittn,TempVec,...
...
[TempVec_ittm2Ext,Humidity_ittm2Ext,TempVecB_ittm2Ext,Meteo_ittm]...
    =helper.CreateVariablesOfPrevious2TimeStep(ittm,ittn,TempVec,Humidity,TempVecB,MeteoDataRaw);
...
if RESPreCalc == 1 || fconvPreCalc==1
    [fconv,rsRoofPreCalc,rsGroundPreCalc,rsTreePreCalc]=resistance_functions.PrecalculateForFasterNumericalSolution(ittn,ittm,...
             ...
             PropOpticalRoof,FractionsRoof,RES,Geometry_m);
else
...
[Ttot,fval,exitflag]=helper.fSolver_Tot(TempVec_ittm,TempVecB_ittm,Humidity_ittm,MeteoData,...
		Int_ittm,ExWater_ittm,Vwater_ittm,Owater_ittm,SoilPotW_ittm,CiCO2Leaf_ittm,TempDamp_ittm,ViewFactor,...
		Geometry_m,ParTree,geometry,FractionsGround,FractionsRoof,...
...
[SWRabsRoofImp,SWRabsRoofVeg,SWRabsTotalRoof,...
		...
		=helper.EB_WB_roof(TR,TB,TempVec_ittm,MeteoData,...
		Int_ittm,ExWater_ittm,Vwater_ittm,Owater_ittm,SoilPotW_ittm,CiCO2Leaf_ittm,Runon_ittm,...
		Geometry_m,FractionsRoof,ParSoilRoof,PropOpticalRoof,ParThermalRoof,ParVegRoof,...
...
[SWRin_t,SWRout_t,SWRabs_t,SWRabsDir_t,SWRabsDiff_t,SWREB_t,albedo_canyon,...
		...
		=helper.EB_WB_canyon(TC,TB,TempVec_ittm,Humidity_ittm,MeteoData,...
		...
		Geometry_m,ParTree,geometry,FractionsGround,...
		WallLayers,ParSoilGround,ParInterceptionTree,...
		PropOpticalGround,PropOpticalWall,PropOpticalTree,...
		ParThermalGround,ParThermalWall,ParVegGround,ParVegTree,...
		SunPosition,HumidityAtm,Anthropogenic,ParCalculation,...
...
[HbuildIntc,LEbuildIntc,GbuildIntc,SWRabsBc,LWRabsBc,TDampGroundBuild,WasteHeat,EnergyUse,HumidBuilding,ParACHeat_t,YBuildInt]=...
    ...
    Geometry_m,PropOpticalIndoors,ParHVAC,ParCalculation,ParThermalBulidingInt,ParWindows,BEM_on,HVACSchedule);
...
[Tmrt,BoleanInSun,SWRdir_Person,SWRdir_in_top,SWRdir_in_bottom,...
	...
	ParTree,ParVegTree,geometry,Geometry_m,SunPosition,Person);
	
[u_ZPerson]=resistance_functions.WindProfile_PointOutput(Person.HeightWind,...
	Geometry_m,ParVegTree,ParTree,MeteoData,FractionsGround,ParVegGround);	
...
%--------------------------------------------------------------------------
Geometry_m_Out(ittm)			=	Geometry_m;
...
Zatm = MeteoData.Zatm;
ParHVAC_Out(ittm).ACon = ParHVACorig.ACon;
ParHVAC_Out(ittm).Heatingon = ParHVACorig.Heatingon;


SwitchOnFigure = 0;

% Plot and calculate radiation and energy balance
[WaterFluxRoof,WaterFluxCan,WaterFluxBuild,WaterFluxUrban]=water_functions.WaterBalanceComponents(MeteoDataRaw,...
    ...
    FractionsGround_Out,geometry_Out,Geometry_m_Out,SwitchOnFigure,ittm);

helper.UrbanClimateVariables(TempVec,UTCI,Results2m,MeteoDataRaw,MeanRadiantTemperature,...
    TempVecB,HumidityBuilding,...
    FractionsGround_Out,FractionsRoof_Out,ParTree_Out,SwitchOnFigure,ittm,BEM_on);

[EnergyFluxUrban,EnergyFluxCan,EnergyFluxRoof]=helper.PlanAreaEnergyBalanceCalculation(ViewFactor,MeteoDataRaw,...
    ...
    geometry_Out,FractionsGround_Out,PropOpticalRoof_Out,Anthropo,Geometry_m_Out,SwitchOnFigure,ittm,BEM_on);
...
%     ...
% 	'Geometry_m_Out','ParTree_Out','geometry_Out','FractionsRoof_Out','FractionsGround_Out',...
...
% else
%     % Option 4: only temperature
% %--------------------------------------------------------------------------
%     save(['CalculationBayOpt',NameOutput],'NameOutput','Solver','TempVec',...
% 	'Results2m','n','m','Name_Site','MeteoDataRaw','Geometry_m_Out','ParTree_Out','geometry_Out','Results2mEnergyFlux','MeanRadiantTemperature','Zatm','UTCI',...
%     'AlbedoOutput','ViewFactor','UTCI',...
%     'TempVecB','ittm','Sensitivity');
% 
%     maxValue = max(Results2m.T2m);
%     maxValue
% end
